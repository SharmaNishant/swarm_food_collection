...
int foodValue1 = 1, foodValue2 = 10,
    penaltyParameter =
        1; // penaltyParameter 1 for speed else for food value deposited
...
int totalObjectsDeposited = 0, towardsObjectFlag = 0, startSimulation = 0,
    ostacleFlag = 0;

float deviateX, deviateY, tempX, tempY, lastX, lastY,
    robotSpeed = 0.75, robotLifeCost = 0, robotDirection;

float colisionAvoidDirection, distanceToSource, neighbourSearchLimit = 2.0,
                                                robotMinimumDistance = 1.0,
                                                robotRepulsionForce = 1.0;
...
// double estimatePatch1X = 100 , estimatePatch1Y = 100, estimatePatch2X = 100,
// estimatePatch2Y = 100;
...
// for toSource
float lastSourceX, lastSourceY, sourceDirection, sourceDetailSaveFlag = 0,
                                                 sourceDistance;
...
// for neighSearch
float neighbourSearchCenterX, neighbourSearchCenterY, neighbourSearchFlag = 0,
...
// function to start or stop the code based on the message received from the
// master node
...
// function to set the simulation details based on the message received from the
// master node
void simulationDetails(const ants2014::simDet::ConstPtr &msg) {
  ...
  // setting boundary value
  {
    ...
    boundary.dv13 =
        ((boundary.x3 - boundary.x1) * (boundary.x3 - boundary.x1)) +
        ((boundary.y3 - boundary.y1) * (boundary.y3 - boundary.y1));
    boundary.dv12 =
        ((boundary.x2 - boundary.x1) * (boundary.x2 - boundary.x1)) +
        ((boundary.y2 - boundary.y1) * (boundary.y2 - boundary.y1));

    //  cout<<boundary.x1<<" "<<boundary.y1<<" "<<boundary.x2<<"
    //  "<<boundary.y2<<" "<<boundary.x3<<" "<<boundary.y3<<" \n";
  }
  ...
  // setting goal value
  {
    ...
    nest.dv13 = ((-nest.x1 + nest.x3) * (-nest.x1 + nest.x3)) +
                ((-nest.y1 + nest.y3) * (-nest.y1 + nest.y3));
    nest.dv12 = ((-nest.x1 + nest.x2) * (-nest.x1 + nest.x2)) +
                ((-nest.y1 + nest.y2) * (-nest.y1 + nest.y2));
    // cout<<nest.x<<" "<<nest.y<<nest.x1<<" "<<nest.y1<<nest.x2<<"
    // "<<nest.y2<<nest.x3<<" "<<nest.y3<<"\n";
  }
}

// function updating other robot locations based on the message received from
// other robots
...
// function updating other object information based on the message received from
// object node
void objectLocator(const ants2014::objLocate::ConstPtr &msg) {
  if (curObj.id == -1) {
    ...
    tempObj.dist =
        sqrt(pow(robotX - tempObj.x, 2) + pow(robotY - tempObj.y, 2));
    if (tempObj.dist < 2.0) {
      curObj = tempObj;
  ...
  //        tempObj.x = msg->X;
  //        tempObj.y = msg->Y;
  //        tempObj.dist = sqrt(pow(robotX - tempObj.x,2) + pow(robotY -
  //        tempObj.y,2)); if(tempObj.dist < curObj.dist)
  //            curObj = tempObj;
  //    }
}
...
int main(int argc, char **argv) {
  ...
  ros::Publisher myLocate =
      n.advertise<ants2014::robLocate>("robotLocation", 10);
  ...
  ros::Subscriber simDet =
      n.subscribe("simulationDetails", 500, simulationDetails);
  ros::Subscriber simCom =
      n.subscribe("simulationCommand", 1000, simulationCommand);
  ros::Subscriber robotLocate =
      n.subscribe("robotLocation", 1000, robotLocator);
  ros::Subscriber objectLocate =
      n.subscribe("objectLocation", 1000, objectLocator);
  ...
  while (ros::ok()) {
    ...
    for (i = 0; i < robotLocationList.size(); i++) {
      if (i == robotID)
        continue;
      robotLocationList[i].dist = sqrt(pow(robotX - robotLocationList[i].x, 2) +
                                       pow(robotY - robotLocationList[i].y, 2));
    }
    ...
    if (state == toObject) {
      ...
      if (curObj.id != -1) {
        ...
        curObj.dist =
            sqrt(pow(robotX - curObj.x, 2) + pow(robotY - curObj.y, 2));
        // cout<<"\nObject located with distance = "<<curObj.dist<<"\n";
        // if object distance is less than this value
        if (curObj.dist <= 2.0) // objMinDist
        {
          // updating cost function
          /*  costFunction[selectedCostID].cost = (0.75 *
            costFunction[selectedCostID].cost) + (0.25 *
            ((distanceToSource/costFunction[selectedCostID].length) *
            (totalTime) + randomWalkTime)); lastStepTime = totalTime;
    ...
    if (state == toNest) {
      ...
      nest.dv1m2 = ((-nest.x1 + robotX) * (-nest.x1 + nest.x2)) +
                   ((-nest.y1 + robotY) * (-nest.y1 + nest.y2));
      nest.dv1m3 = ((-nest.x1 + robotX) * (-nest.x1 + nest.x3)) +
                   ((-nest.y1 + robotY) * (-nest.y1 + nest.y3));

      // test if the robot has entered the nest
      if ((0 < nest.dv1m2) && (nest.dv1m2 < nest.dv12) && (0 < nest.dv1m3) &&
          (nest.dv1m3 < nest.dv13)) {
        // saving depositing point value
        tempX = robotX;
        tempY = robotY;
        /* if(curObj.patch == 0)
             patchFunctionList[0].cost = (0.9 * patchFunctionList[0].cost) +
         (0.1 * (( (totalTime) / (curObj.value * patchFunctionList[0].survival))
         + randomWalkTime)); else patchFunctionList[1].cost = (0.9 *
         patchFunctionList[1].cost) + (0.1 * (( (totalTime) / (curObj.value *
         patchFunctionList[1].survival)) + randomWalkTime));
        ...
        // cout<<"\nPatch 1 Cost : "<<patchFunctionList[0].cost<<", Patch 2 Cost
        // : "<<patchFunctionList[1].cost<<"\n";

        robotLifeCost = ((1 - alpha) * robotLifeCost) +
                        (alpha * (((totalTime) / (curObj.value)) +
                                  (beta * randomWalkTime)));
        ...
        tradeOffValue = (robotLifeCost / totalObjectsDeposited) *
                        (ros::Time::now().toSec() - startTime);

        cout << "\nTrade off Value = " << tradeOffValue << "\n\n";

        patchFunctionList[curObj.patch].cost =
            (100 * curObj.value * patchFunctionList[curObj.patch].survival) /
            (sqrt(pow(curObj.x - robotX, 2) + pow(curObj.y - robotY, 2)));
        patchFunctionList[curObj.patch].estimateX = curObj.x;
        patchFunctionList[curObj.patch].estimateY = curObj.y;

        cout << "net Cost p1 " << patchFunctionList[0].cost << " | net Cost p2 "
             << patchFunctionList[1].cost << "\n\n";
        ...
        //   file<<"Cost : "<< robotLifeCost<<" | lastStep Ts&Tp : "<< totalTime
        //   <<" | randWalkTime : "<<randomWalkTime<<" | foodValue :
        //   "<<curObj.value<<" | OverallTime : "<<((ros::Time::now().toSec() -
        //   startTime))<<" | totalObjectsDeposited :
        //   "<<totalObjectsDeposited<<" | netFoodValue 1 :
        //   "<<patchFunctionList[0].cost<<" | netFoodValue 2 :
        //   "<<patchFunctionList[1].cost<<" | tradeOffValue
        //   :"<<tradeOffValue<<" | tradeOFF : "<<tradeOffFlag<<" |
        //   lastSelectedPatch : "<<curObj.patch<<" | robotSpeed :
        //   "<<robotSpeed<<" |\n";
        file << robotLifeCost << " " << totalTime << " " << randomWalkTime
             << " " << ((ros::Time::now().toSec() - startTime)) << " "
             << totalObjectsDeposited << " " << tradeOffValue << " "
             << tradeOffFlag << " " << curObj.patch << " " << robotSpeed
             << "\n";
        totalTime = 0;
        randomWalkTime = 0;
        ...
        /*{
            ...
            else if((patchFunctionList[0].cost / ((patchFunctionList[0].object *
        patchFunctionList[0].survival))) >
        (patchFunctionList[1].cost/((patchFunctionList[1].object *
        patchFunctionList[1].survival))))
            {
                state = toSource;
                lastSourceX = patchFunctionList[1].estimateX;
                lastSourceY = patchFunctionList[1].estimateY;
      ...
      distanceTravelled =
          sqrt(pow(lastSourceX - robotX, 2) + pow(lastSourceY - robotY, 2));
    }
    ...
    // position update while Searching the neighbourhood of the predict Source
    // Location
    ...
    // now actual step change as calculated in the state above
    {
      ...
      for (i = 0; i < robotLocationList.size(); i++) {
        if (robotLocationList[i].dist < robotMinimumDistance) {
          ostacleFlag = 1;
          colisionAvoidDirection = atan2((robotY - robotLocationList[i].y),
                                         (robotX - robotLocationList[i].x));
          deviateX += ((robotRepulsionForce - robotLocationList[i].dist) *
                       cos(colisionAvoidDirection));
          deviateY += ((robotRepulsionForce - robotLocationList[i].dist) *
                       sin(colisionAvoidDirection));
          ros::Duration(0.2).sleep();
        }
      }

      if (state == randWalk) {
        thisRobotLocation.X = robotX =
            (robotX + (robotSpeed * cos(robotDirection)) + deviateX);
        thisRobotLocation.Y = robotY =
            (robotY + (robotSpeed * sin(robotDirection)) + deviateY);
        randomWalkTime += 0.2;
      }

      if (state == toSource) {
        thisRobotLocation.X = robotX =
            (robotX + (robotSpeed * cos(sourceDirection)) + deviateX);
        thisRobotLocation.Y = robotY =
            (robotY + (robotSpeed * sin(sourceDirection)) + deviateY);
        totalTime += 0.2;
      }
      ...
      if (state == neighSearch) {
        ...
        if ((sqrt(pow(neighbourSearchCenterX - robotX, 2) +
                  pow(neighbourSearchCenterY - robotY, 2))) >
            neighbourSearchLimit) {
          robotX = lastX;
          robotY = lastY;
          continue;
        }
      ...
      boundary.dv1m3 = ((robotX - boundary.x1) * (boundary.x3 - boundary.x1)) +
                       ((robotY - boundary.y1) * (boundary.y3 - boundary.y1));
      boundary.dv1m2 = ((robotX - boundary.x1) * (boundary.x2 - boundary.x1)) +
                       ((robotY - boundary.y1) * (boundary.y2 - boundary.y1));

      // cout<<"boudn values check\n"<<boundary.dv12<<"  "<<boundary.dv1m2<<"
      // "<<boundary.dv13<<"  "<<boundary.dv1m3<<"\n";
      if (!((0 < boundary.dv1m2) && (boundary.dv1m2 < boundary.dv12)) ||
          !((0 < boundary.dv1m3) && (boundary.dv1m3 < boundary.dv13))) {
    ...
    // file<<"Time :"<<secs<<" | POS x : "<<thisRobotLocation.X<<" | POS y :
    // "<<thisRobotLocation.Y<<" | State : "<<state<<" | Cost 1 :
    // "<<patchFunctionList[0].cost<<" | Cost 2 :
    // "<<patchFunctionList[1].cost<<" | Patch 1 Visit : " <<
    // patchFunctionList[0].visit <<" | Patch 2 Visit : "<<
    // patchFunctionList[1].visit <<" | totalFood : "<<
    // totalObjectsDeposited<<"|\n";;
    // file<<"|" << patchFunctionList[0].visit <<"|"<<
    // patchFunctionList[1].visit <<"|\n";
    file.flush();
  }
}
