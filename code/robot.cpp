...
float X, Y, lastX, lastY, robotSpeed=0.001, direction, objMin, robMin, colDir, disToSource;
...
ros::Subscriber objectLocate = n.subscribe("objectLocation", 1000, objectLocator);
  ...
  while (ros::ok())
  {
    ...
    {
        cout<<"Searching the neighbourhood of the predict Source Location";
        if(nsFlag==0)
        {
    ...
    //now actual step change as calculated in the state above
    {
        ...
        if(state==neighSearch)
        {
            myLoc.X = marker.pose.position.x += robotSpeed+cos(direction) + X;
            myLoc.Y = marker.pose.position.y += robotSpeed+sin(direction) + Y;
            if((sqrt(pow(centerX-marker.pose.position.x,2) + pow(centerY-marker.pose.position.y,2))) > 2 )
            {
                continue;
            }
        ...
        if(!((0< boundary.dv1m2 < boundary.dv12) && (0<boundary.dv1m3<boundary.dv13)))
        {
