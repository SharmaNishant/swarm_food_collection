...
int totObjectsDeposited = 0, statObj = 0, totRobots = 0, bound_flag = 0,
    state = randWalk, startFlag = 0, obstFlag = 0, toFlag = 0, rwFlag = 0;
float X, Y, tempX, tempY, tempDist, lastX, lastY,
    robotSpeed = 0.001, direction, objMin, robMin, colDir, disToSource,
    nSLimit = 2.0, robMinDist = 1.0, robRepForce = 1.0;
double secs;

float leviDist, leviX, leviY, leviDir, leviFlag = 0;
...
int main(int argc, char **argv) {
  ...
  ros::Publisher myLocate =
      n.advertise<task_part_sim::robLocate>("robotLocation", 10);
  ros::Publisher robObject =
      n.advertise<task_part_sim::robObj>("robObject", 10);
  ...
  while (ros::ok()) {
    ...
    for (i = 0; i < objLoc.size(); i++) {
      objLoc[i].dist = sqrt(pow(marker.pose.position.x - objLoc[i].x, 2) +
                            pow(marker.pose.position.y - objLoc[i].y, 2));
    ...
    for (i = 0; i < roboLoc.size(); i++) {
      if (i == robotID)
        continue;
      roboLoc[i].dist = sqrt(pow(marker.pose.position.x - roboLoc[i].x, 2) +
                             pow(marker.pose.position.y - roboLoc[i].y, 2));
    }
    ...
    if (state == randWalk) {
      if (leviFlag == 0) {
        leviX = (rand() % 50) - 25;
        leviY = (rand() % 50) - 25;
        leviDir = atan2((leviY - marker.pose.position.y),
                        (leviX - marker.pose.position.x));
        leviFlag = 1;
        // eviDist=sqrt(pow(leviX-marker.pose.position.x,2) +
        // pow(leviY-marker.pose.position.y,2));
      }
      disTrav = sqrt(pow(leviX - marker.pose.position.x, 2) +
                     pow(leviY - marker.pose.position.y, 2));
      if (rwFlag == 1 || disTrav < 1.5) {
        state = leviSearch;
        rwFlag = 0;
        nsFlag = 0;
        disTrav = 0;
    ...
    if (state == leviSearch) {
      cout << "Searching the neighbourhood\n";
      if (nsFlag == 0) {
        leviX = marker.pose.position.x;
        leviY = marker.pose.position.y;
        nsFlag = 1;
        nsTime = 0;
      }
    ...
    if (state == toObject) {
      ...
      if (curObj.id != -1) {
        direction = atan2((curObj.y - marker.pose.position.y),
                          (curObj.x - marker.pose.position.x));
        if (curObj.dist <= 0.5) // objMinDist
        {
          myCF[selCostID].cost =
              (0.75 * myCF[selCostID].cost) +
              (0.25 *
               ((disToSource / myCF[selCostID].length) * (TTime) + rwTime));
          lastTime = TTime;
          lastRwTime = rwTime;
          rwTime = 0;
          TTime = 0;
    ...
    if (state == toNest) {
      ...
      direction = atan2((nest.y - marker.pose.position.y),
                        (nest.x - marker.pose.position.x));
      if (disTrav >= selLen) {
        robObjct.id = robotID;
        // myCF[selCostID].cost= (0.75 * myCF[selCostID].cost) +
        // (0.25*((disToSource/myCF[selCostID].length) * (TTime) + rwTime));
      ...
      nest.dv1m2 =
          ((-nest.x1 + marker.pose.position.x) * (-nest.x1 + nest.x2)) +
          ((-nest.y1 + marker.pose.position.y) * (-nest.y1 + nest.y2));
      nest.dv1m3 =
          ((-nest.x1 + marker.pose.position.x) * (-nest.x1 + nest.x3)) +
          ((-nest.y1 + marker.pose.position.y) * (-nest.y1 + nest.y3));
      ...
      if ((0 < nest.dv1m2) && (nest.dv1m2 < nest.dv12) && (0 < nest.dv1m3) &&
        ...
        // myCF[selCostID].cost= (0.75 * myCF[selCostID].cost) +
        // (0.25*((disToSource/myCF[selCostID].length) * (lastTime) +
        // lastRwTime));
      ...
      marker.color = blue;
      // disTrav+=robotSpeed;
      disTrav = sqrt(pow(lastSourceX - marker.pose.position.x, 2) +
                     pow(lastSourceY - marker.pose.position.y, 2));
      // cout<<"to nest data \n"<<nest.dv1m2<<"  "<<nest.dv12<<"
      // "<<nest.dv1m3<<"  "<<nest.dv13<<"\n\n  ";
    }
    ...
    if (state == toSource) {
      ...
      if (sdFlag == 0) {
        sdFlag = 1;
        sourceDirection = atan2((lastSourceY - marker.pose.position.y),
                                (lastSourceX - marker.pose.position.x));
        sourceDistance = sqrt(pow(lastSourceX - marker.pose.position.x, 2) +
                              pow(lastSourceY - marker.pose.position.y, 2));
      ...
      marker.color = yellow;
      disTrav = sqrt(pow(tempX - marker.pose.position.x, 2) +
                     pow(tempY - marker.pose.position.y, 2));
    }
    ...
    // now actual step change as calculated in the state above
    {
      for (i = 0; i < roboLoc.size(); i++) {
        if (roboLoc[i].dist < robMinDist) {
          obstFlag = 1;
          colDir = atan2((marker.pose.position.y - roboLoc[i].y),
                         (marker.pose.position.x - roboLoc[i].x));
      ...
      if (state == toSource) {
        myLoc.X = marker.pose.position.x +=
            robotSpeed + cos(sourceDirection) + X;
        myLoc.Y = marker.pose.position.y +=
            robotSpeed + sin(sourceDirection) + Y;
        TTime += 0.2;
      }
      if (state == toObject) {
        myLoc.X = marker.pose.position.x += robotSpeed + cos(direction) + X;
      ...
      if (state == leviSearch) {
        myLoc.X = marker.pose.position.x += robotSpeed + cos(direction) + X;
        myLoc.Y = marker.pose.position.y += robotSpeed + sin(direction) + Y;
        if ((sqrt(pow(leviX - marker.pose.position.x, 2) +
                  pow(leviY - marker.pose.position.y, 2))) > 4.0) {
      ...
      if (state == neighSearch) {
        myLoc.X = marker.pose.position.x += robotSpeed + cos(direction) + X;
        myLoc.Y = marker.pose.position.y += robotSpeed + sin(direction) + Y;
        if ((sqrt(pow(centerX - marker.pose.position.x, 2) +
                  pow(centerY - marker.pose.position.y, 2))) > nSLimit) {
          marker.pose.position.x = lastX;
          marker.pose.position.y = lastY;
          continue;
        }
      ...
      boundary.dv1m3 = ((marker.pose.position.x - boundary.x1) *
                        (boundary.x3 - boundary.x1)) +
                       ((marker.pose.position.y - boundary.y1) *
                        (boundary.y3 - boundary.y1));
      boundary.dv1m2 = ((marker.pose.position.x - boundary.x1) *
    ...
    file << "Time :" << secs << " | POS x : " << myLoc.X
         << " | POS y : " << myLoc.Y << " | State : " << state << " |\n";
    file.flush();
  }
}
