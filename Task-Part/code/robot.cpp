...
float deviateX, deviateY, tempX, tempY, lastX, lastY, robotSpeed = 0.001,
                                                      robotDirection;
...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    if (state == toObject) {
      ...
      if (curObj.id != -1) {
        ...
        if (curObj.dist <= 2.0) // objMinDist
        {
          ...
          costFunction[selectedCostID].cost =
              (0.75 * costFunction[selectedCostID].cost) +
              (0.25 *
    ...
    // now actual step change as calculated in the state above
    {
      ...
      if (state == randWalk) {
        thisRobotLocation.X = robotX +=
            robotSpeed + cos(robotDirection) + deviateX;
        thisRobotLocation.Y = robotY +=
            robotSpeed + sin(robotDirection) + deviateY;
        randomWalkTime += 0.2;
      }

      if (state == toSource) {
        thisRobotLocation.X = robotX +=
            robotSpeed + cos(sourceDirection) + deviateX;
        thisRobotLocation.Y = robotY +=
            robotSpeed + sin(sourceDirection) + deviateY;
        totalTime += 0.2;
      }
    ...
    file << "Time :" << secs << " | POS x : " << thisRobotLocation.X
         << " | POS y : " << thisRobotLocation.Y << " | State : " << state
         << " |\n";
    file.flush();
  }
}
